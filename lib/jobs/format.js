...
function process(raw_data) {
    if (!raw_data || !raw_data.result || !raw_data.result.job) {
        return false;
    }
    ...
    raw_data.result.job.forEach(function(job) {

        //XML Parser returns string values wrapped in an
        //array. This strips them away, and leaves strings
        for(var prop in job) {
          if(job[prop] instanceof Array) job[prop] = job[prop][0];
        }
